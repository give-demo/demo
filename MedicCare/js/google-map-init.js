var CODECLONE_googlemap_init_obj = {};
var CODECLONE_GEOCODE_ERROR = "Error";
// Google map Styles
var CODECLONE_googlemap_styles = {
    'default': [],
...
function googlemap_init(dom_obj, coords) {
    ...
    try {
        ...
        var id = dom_obj.id;
        CODECLONE_googlemap_init_obj[id] = {};
        CODECLONE_googlemap_init_obj[id].dom = dom_obj;
        CODECLONE_googlemap_init_obj[id].point = coords.point;
        CODECLONE_googlemap_init_obj[id].description = coords.description;
        CODECLONE_googlemap_init_obj[id].title = coords.title;
        CODECLONE_googlemap_init_obj[id].opt = {
            zoom: coords.zoom,
            center: latlng,
            scrollwheel: false,
            scaleControl: false,
            ...
            styles: CODECLONE_googlemap_styles[coords.style ? coords.style : 'default'],
        ...
        custom_map.geocode(coords.latlng!=='' ? {'latLng': latlng} : {"address": coords.address}, function(results, status) {
            if (status == google.maps.GeocoderStatus.OK) {
                CODECLONE_googlemap_init_obj[id].address = results[0].geometry.location;
                googlemap_create(id);
            } else {
                alert(CODECLONE_GEOCODE_ERROR + ' ' + status);
            }
        });

        jQuery(window).resize(function() {
            if (CODECLONE_googlemap_init_obj[id].map) {
                CODECLONE_googlemap_init_obj[id].map.setCenter(CODECLONE_googlemap_init_obj[id].address);
            }
        });
...
function googlemap_create(id) {
    "use strict";
    if (!CODECLONE_googlemap_init_obj[id].address) { 
        return false;
    }
    CODECLONE_googlemap_init_obj[id].map = new google.maps.Map(CODECLONE_googlemap_init_obj[id].dom, CODECLONE_googlemap_init_obj[id].opt);
    CODECLONE_googlemap_init_obj[id].map.setCenter(CODECLONE_googlemap_init_obj[id].address);
    var markerInit = {
        map: CODECLONE_googlemap_init_obj[id].map,
        position: CODECLONE_googlemap_init_obj[id].address   //CODECLONE_googlemap_init_obj[id].map.getCenter()
    };
    if (CODECLONE_googlemap_init_obj[id].point) { 
        markerInit.icon = CODECLONE_googlemap_init_obj[id].point;
    }
    if (CODECLONE_googlemap_init_obj[id].title) { 
        markerInit.title = CODECLONE_googlemap_init_obj[id].title;
    }
    var marker = new google.maps.Marker(markerInit);
    var infowindow = new google.maps.InfoWindow({
        content: CODECLONE_googlemap_init_obj[id].description
    });
    google.maps.event.addListener(marker, "click", function() {
        infowindow.open(CODECLONE_googlemap_init_obj[id].map, marker);
    });
}


function googlemap_refresh(){
    for(id in CODECLONE_googlemap_init_obj){
        googlemap_create(id);
    }
}
